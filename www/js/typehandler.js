...
class TypeHandler{
  ...
  async addNewExecute(e){
    ...
    await mscp.add(this.folderView.path, typeInput.val(), titleInput.val())
    this.folderView.showFolder(this.folderView.path)
  ...
  async addExistingExecute(e){
    ...
    await mscp.add(this.folderView.path, typeInput.val(), titleInput.val(), identifierInput.val())
    this.folderView.showFolder(this.folderView.path)
    e.parents(".toolbarbutton").removeClass("open")
    typeInput.val("folder")
    titleInput.val("")
